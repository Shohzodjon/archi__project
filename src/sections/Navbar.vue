...
<template>
  <nav class="fixed top-0 left-0 right-0 w-full z-[999]">
          ...
          <ul class=" hidden md:flex items-center gap-8">
            <li>
              <a href="+998 90 124 74-74" class="flex items-center gap-2">
                <Phone />
                <span class="font-medium text-white-900 text-base lg:text-[18px] lg:leading-[23.4px]">+998 90 124 74-74</span>
              </a>
            </li>
            <li>
    ...
    <!-- navbar header -->
    <!--  :class="[route.path == '/' ? '' : '!bg-[#154FA1]']" -->
    <div class="py-4 md:py-5 lg:py-7 navbar__bottom !bg-[#154FA1]" >
      <div class="container">
        <div class="flex items-center justify-between">
          <RouterLink to="/"><img :src="logo" alt="logo img" class="w-20 md:w-24 lg:w-24 max-w-[156.6px] max-h-12" /></RouterLink>
          <!-- begin router links  -->
          <ul class="hidden lg:flex items-center gap-5 xl:gap-10">
            ...
            <li>
              <RouterLink to="/blog" class="text-white-900 text-base xl:text-[18px] leading-[23.4px] font-medium font-gilroy-medium">{{
          ...
          <!--  end of router link -->
          <div>
            <BaseButton :content="$t('btn-content.submit')" class="nav__btn  lg:py-2 lg:px-3  xl:px-5 2xl:px-[38px] text-white-900 hidden lg:flex">
              <Arrow />
            </BaseButton>
            <Menu class="cursor-pointer block lg:hidden" @click="toggleFunc"/>
          </div>
        </div>
        <div class="navbar__bottom__line w-full h-[1.5px] bg-[rgba(255,255,255,.3)] mt-4 xl:mt-7 relative z-[999] hidden lg:block"
          :class="[route.path == '/' ? '' : 'hidden']"></div>
      </div>
    </div>
    <!-- end of navbar bottom -->
  </nav>
<div class="fixed flex lg:hidden justify-end top-0 w-full h-full left-0 z-[9999] bg-[rgba(13,64,136,0.4)] " @click="toggleFunc" :class="[show?'active__responsive':'unactive__responsive']">
  <ul class="bg-white-900 pt-20 pb-6 pl-6 pr-4 flex flex-col gap-2 w-60 relative" >
    ...
    <li>
      <RouterLink to="/blog" class="text-blue-500 text-base xl:text-[18px] leading-[23.4px] font-medium font-gilroy-medium responsive__link">{{
        $t('router-link.blog') }}</RouterLink>
    </li>
    <li>
      <RouterLink to="/company"
